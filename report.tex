...
Now, we are going to give a proof that MILPs admits a solution if and only if
they admit a solution of small size. In what follows, we will focus on ILPs, but
the results can be easily extended to MILPs. Most of the definitions and
results can be found in Schrijver's book \cite{Schrijver1998}.

\begin{definition}[Finitely Generated Cone]
  A set of points $C$ is a \textup{finitely generated cone}
  iff $C = \cone~\{x_0, ..., x_{n-1}\}$ where
  $$\cone~\{x_0, ..., x_{n-1}\} =
      \left\{\sum_{i<n} \lambda_i x_i~|~
...
\begin{definition}[Polytope]
  ...
  $$C = \left\{
          \sum_{i<m} \lambda_i x_i~|~\lambda_0, ..., \lambda_{m-1} \geqslant 0
                                     \wedge \sum_{i<m} \lambda_i = 1\right\}$$
\end{definition}
...
\begin{proof}
  ...
  This why, without loss of generality, we can assume that the $y_i$'s are
  integral.

  Let us define
  $$B = \left\{\sum_{i < s} \mu_i y_i~|~0 \leqslant \mu_i \leqslant 1\right\}$$
...
\begin{theorem}[Generalized Farkas-Minkowsky-Weyl Theorem]
  ...
  $A \in \llbracket -m, m \rrbracket^{p \times n}$ with $m \leqslant 1$,
...
As we have presented a method to solve MILPs, we can focus on the main work that
has been done during this internship: the formalization of the previous theorems
and the implementation of an MILP solver.

%TODO: Find a better section name
\subsection{Formalization of Results concerning Linear Inequalities}
The first part of the work consisted in formalizing the results from Schijver's
book, following the description given in the subsection \ref{schrijverbnd}.
Also, other results including the Farkas' Lemma or the Carathéodory's theorem
were formalized.

% 5551 lines of code
The work was published in AFP \cite{Linear_Inequalities-AFP}.
