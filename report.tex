...
% TODO: re-organize the next subsections this way:
% - a subsection about linear and integral linear arithmetic
% - a subsection about SMT-solving and incremental interface
...
% TODO: give a simpler example, and move completely this into the appendix
...
\textit{TODO: complete}
\begin{itemize}
  \item René developed results about integral and bounded vectors or matrices
    and ...
...
defined this way:
$$\mathtt{lincomb\_list}~c~V_s = \sum_{i = 0}^{l-1} c(i) \cdot v_i$$
...
%TODO: talk about \vDash_cs and \vDash_mcs
I first developped a function $\mathtt{branch\_and\_bound\_core}$ that takes
four arguments:
\begin{itemize}
  \item A list of linear constraints \texttt{cs}.
  \item A list of variables supposed to be integral \texttt{Is}.
  \item A function \texttt{lb} mapping variables in \texttt{Is} to a lower
    bound.
  \item A function \texttt{ub} mapping variables in \texttt{Is} to an upper
    bound.
\end{itemize}
The code of this function can be found in appendix \ref{bbcode}

The idea of the arguments \texttt{lb} and \texttt{ub} are to bound the problem.
...
to natural numbers. If we prove that at each recursive call the measure strictly
decreases, then there is no way that a infinite sequence of recursive calls can
happen and the function always terminates. The measure I used is the following:
$$\max\left(
       0,
       \sum_{x_i \in \mathtt{Is}} (\mathtt{ub}(x_i) - \mathtt{lb}(x_i))
       \right)$$
% Should I give more details about the termination prove or is it enough?
...
complete solver for MILPs. The problem is that linear constraints are coded with
a datastructure called \textit{linear polynomials} that should be converted to
the form of vectors and matrices to use the theorem.
...
After some work around converting rational constraints to integral constraints,
I could define a function \texttt{compute\_bound} that maps a list of
constraints to the desired bound and prove that:
\Snippet{compute-bound}

Finally, I was able to state a complete MILP-solver based on the
...
\subsection{Using the incremental solver}
