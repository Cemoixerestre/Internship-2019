...
This kind of equivalence is not always trivial to prove. For example, it took me
around 300 lines of code to prove the previous result.
...
After some work to convert rational constraints to integral constraints,
...
rewritten so that arbitrary constraints could be asserted.

\subsection{Gomory Cuts}

Suppose that a valuation $v$ such that $v \vDash_I S$ has been found but that
...
A method to obtain cuts is described in \cite[Section 4]{Dutertre2006}. These
cuts are called \textit{Gomory cuts}. The combination of the branch-and-bound
algorithm with cut generation is called \textit{branch-and-cut} and a dramatic
acceleration of the branch-and-bound algorithm has been observed.
